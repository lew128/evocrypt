...
Additionally, it is easy to change or add to this program to further
make a unique program by adding to the lists of PRNGs or hash functions,
or changing the mechanisms by which the password is used to select
values for the various uses throughout the program.

The program generates new versions of itself, so other pairs
can begin their independent evolution. Every new version will be unique,
only able to work with copies of itself.

This version of the program is a standard Unix command-line utility.

On startup :

0) It reads the password and destination and 'encrypt' or 'decrypt' from
the command line, assumes that stdin and stdout are source and destination.

1) Uses the password to select the initialization of the pseudorandom
number generator, one for sender function and the other for the receiver
function.

2) For both encryption and decryption, it takes the input text from stdin,
xors it with the output of the prng. 

Linux command line utilities are fine for files.

For interative communications, we need to add another level, this is
evochat.py, not completed yet and has an annoying bug. That is not part
of this initial release, tho the code may be there.
...
OTOH, I know how to compute entropy, and XOR is theoretically solid, a
bit-level modular addition.  Given enough different PRNGs and hashes to
mask the original signal, I can then dilute that cryptotext with output
of other PRNGs, mixing controled by yet other PRNGs. Thusly, the
bit-rate of plain text in the crypto-text + random data string can be
made as low as required by any level of paranoia.  (I haven't put that
last bit in, it is the last to be done, after I am sure the rest of the
code is solid.)
...
the power to break my best effort. 

As a community, we have 2 more tactics to counter NSA, both forms of
evolution.
...
functionally correct before they are added to the appropriate
list. Every such function increases the problem of decryption, although
it may not change the bit rate of plain text in cipertext.
...
In biological evolution,tac

program is generated by sender and receiver, which replaces the one
currently in use.  The new program is generated by :
...
After we have a random number with their local sysem's entropy
in it, we can begin another PRNG initialized with that random
number.  This is another layer of protection, recursive if
necessary.

Seed should have entropy added from the local machine to make it less
likely that a seed is re-used, which would allow relatively
easy deciphering of both first and 2nd messages.
...
def generate_new_program( password, this_file_name, new_file_name,
    ...
    Scrambles everything scramblable in the text and writes the program
        to this_file_name.
    """
    print( "generate_new_program : password = '" + password + "'" )
    print( "generate_new_program : new_file_name = '" + new_file_name + "'" )
    ...
    print( "folded_hash = ", folded_hash, "len = ", len( new_randoms_program)  )
...
def encrypt_file( to_be_encrypted_file_name, password, paranoia_level ) :
    """
...
def decrypt_file( to_be_decrypted_file_name, password, paranoia_level ) :
    """
...
#    print( '#' + str( sys.argv[ 1 : ] ) )
    ...
    for o, a in OPTS :
        ...
        if o in ( "--decrypt" ) or o in ( "-d" ) :
            FILE_NAME = a
            decrypt_file( FILE_NAME, PASSWORD, PARANOIA_LEVEL )

        if o in ( "--encrypt" ) or o in ( "-e" ) :
            FILE_NAME = a
            encrypt_file( FILE_NAME, PASSWORD, PARANOIA_LEVEL )
